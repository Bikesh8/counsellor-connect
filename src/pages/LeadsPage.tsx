...
import { Search, Plus, SlidersHorizontal, MoreHorizontal, Phone, Mail, ChevronDown, Eye, Edit, MessageSquare, CalendarClock } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <Button className="mt-3 sm:mt-0 gap-2">
                ...
                <td className="py-3 px-3">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem className="gap-2 text-xs"><Eye className="w-3.5 h-3.5" />View Lead</DropdownMenuItem>
                      <DropdownMenuItem className="gap-2 text-xs"><Edit className="w-3.5 h-3.5" />Update Stage</DropdownMenuItem>
                      <DropdownMenuItem className="gap-2 text-xs"><MessageSquare className="w-3.5 h-3.5" />Add Note</DropdownMenuItem>
        ...
        {filtered.map((lead) => (
          <div key={lead.id} className={`bg-card rounded-lg border border-border p-4 ${lead.isOverdue ? "border-destructive/30" : ""}`}>
            ...
            <div className="text-xs text-muted-foreground mb-2">
              GPA: {lead.gpa} · {lead.qualification} · {lead.testScore}
            </div>
